...
def classify_quadrant(scores: NormalizedScores, custom_thresholds: dict = None) -> tuple:
    DEFAULT_THRESHOLDS = {
...
def generate_optimization_strategy(quadrant: str, industry: str, persona: str, openai_api_key: str = None) -> dict:
    ...
    # Handle case where quadrant might not be in dictionary keys
    if quadrant not in base_strategies:
        return {
            "core_focus": "General optimization recommended.",
            "general_actions": ["Analyze market position.", "Review customer feedback."],
            "note": "Quadrant strategy not found in base definitions."
        }
    ...
    if openai_api_key:
        try:
            ...
            """
            # Note: Using older openai syntax as per your snippet. 
            # If using openai>=1.0.0, this syntax changes to openai.chat.completions.create
...
def render_task_tracker():
    ...
    if st.session_state.tasks:
        # FIX: Iterate over a copy of the list or indices to allow safe deletion
        for idx, task in enumerate(st.session_state.tasks):
            col1, col2, col3 = st.columns([0.7, 0.2, 0.1])
            with col1:
                st.write(f"**{task['task_name']}** | Due: {task['due_date']}")
            with col2:
                # Checkbox updates state directly
                st.checkbox("Mark Complete", key=f"task_{idx}", value=task['completed'])
            
            # Sync checkbox state back to task list
            if st.session_state[f"task_{idx}"]:
...
def generate_pdf_report(quadrant: str, scores: NormalizedScores, strategy: dict, tasks: list) -> io.BytesIO:
    ...
    if "ai_enhanced_tactics" in strategy:
        pdf.ln(3)
        pdf.set_font("Arial", "B", 12)
        pdf.cell(0, 7, "AI Enhanced Tactics:", ln=True)
        pdf.set_font("Arial", "", 11)
        # FIX: Handle encoding issues for PDF with special characters
        tactics_text = strategy.get("ai_enhanced_tactics", "").encode('latin-1', 'replace').decode('latin-1')
        pdf.multi_cell(0, 6, tactics_text)
    ...
    if tasks:
        for t in tasks:
            ...
            pdf.cell(0, 7, f"- {t.get('task_name')} | Due: {t.get('due_date')} | {status}", ln=True)
    else:
...
def main():
    ...
    render_performance_chart(demo_scores)
    ...
    # Filter tasks for the report if needed (optional, currently passing all)
    pdf_buf = generate_pdf_report(quadrant, demo_scores, strategy, st.session_state.tasks)
    
    st.download_button(
        "Download Demo Report (PDF)", 
        data=pdf_buf, 
        file_name="sales_matrix_report.pdf", 
        mime="application/pdf"
    )

# FIX: Corrected syntax for main execution block
